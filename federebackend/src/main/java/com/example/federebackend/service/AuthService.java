...
public class AuthService {
     ...
     * Works for Patient and Medecin.
     */
    public MessageResponse forgotPassword(String email) {
        Utilisateur utilisateur = utilisateurRepository.findByEmail(email)
                .orElseThrow(() -> new RuntimeException("Aucun compte trouvé avec cet email"));

        // Only allow Patient and Medecin to reset password
        if (utilisateur.getTypeUtilisateur() != TypeUtilisateur.Patient &&
                utilisateur.getTypeUtilisateur() != TypeUtilisateur.Medecin) {
            throw new RuntimeException("Réinitialisation non autorisée pour ce type de compte");
        }
    ...
    public MessageResponse resetPassword(String token, String newPassword) {
        ...
        if (email == null) {
            throw new RuntimeException("Le lien de réinitialisation est invalide ou a expiré");
        }
    ...
    /**
     * Validate a reset token (check if it is still valid).
     */
