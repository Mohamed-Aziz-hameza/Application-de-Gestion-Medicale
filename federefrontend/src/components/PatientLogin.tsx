      ...
      <div style={{flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'linear-gradient(90deg, #f6fbff 0%, #fff 100%)'}}>
        <form onSubmit={handleSubmit} style={{background: '#fff', borderRadius: 22, boxShadow: '0 8px 40px rgba(15,23,42,0.13)', padding: '44px 38px 32px 38px', maxWidth: 480, width: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center'}}>
          <div style={{display: 'flex', flexDirection: 'column', alignItems: 'center', marginBottom: 24}}>
            ...
            <h1 style={{margin: 0, fontWeight: 900, fontSize: 28}}>Connexion Patient</h1>
            <span style={{color: '#64748b', fontSize: 15}}>Accédez à votre espace personnel</span>
          </div>
          ...
          <div style={{width: '100%', marginBottom: 18, position: 'relative'}}>
            ...
            <span style={{position: 'absolute', right: 44, top: 16, color: '#a3a3a3', fontSize: 14, cursor: 'pointer'}} onClick={() => alert('Mot de passe oublié ? (à implémenter)')}>Mot de passe oublié ?</span>
          </div>
          {error && <div style={{color: '#ec4899', marginBottom: 10, fontWeight: 600}}>{error}</div>}
          <button type="submit" style={{background: 'linear-gradient(90deg, #ec4899 0%, #f472b6 100%)', color: '#fff', border: 'none', borderRadius: 12, padding: '14px 0', width: '100%', fontWeight: 700, fontSize: 17, marginBottom: 18, boxShadow: '0 8px 24px rgba(236,72,153,0.13)', cursor: 'pointer'}}>
            Se connecter &rarr;
          </button>
