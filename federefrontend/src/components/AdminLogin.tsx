...
import { FaArrowLeft, FaEnvelope, FaLock, FaEye, FaEyeSlash, FaShieldAlt, FaCheckCircle, FaExclamationTriangle } from "react-icons/fa";
import logo from "../assets/logo.png";
import "./AdminLogin.css";

const DEMO_EMAIL = "admin@medicare.com";
      ...
      <div style={{flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'linear-gradient(90deg, #f6fbff 0%, #fff 100%)'}}>
        <form onSubmit={handleSubmit} style={{background: '#fff', borderRadius: 22, boxShadow: '0 8px 40px rgba(15,23,42,0.13)', padding: '44px 38px 32px 38px', maxWidth: 480, width: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center'}}>
          ...
          <div style={{width: '100%', marginBottom: 18}}>
            <label style={{fontWeight: 600, color: '#222', fontSize: 15, marginBottom: 6, display: 'block'}}>Identifiant administrateur</label>
            <div style={{display: 'flex', alignItems: 'center', background: '#f1f5f9', borderRadius: 10, padding: '0 14px', height: 48, marginBottom: 6}}>
              <FaEnvelope style={{color: '#94a3b8', fontSize: 18, marginRight: 8}} />
              <input
                type="email"
                value={email}
                onChange={e => setEmail(e.target.value)}
                placeholder="admin@medicare-plus.tn"
          ...
          <button type="submit" style={{width: '100%', background: 'linear-gradient(90deg, #16a34a, #22c55e)', color: '#fff', border: 'none', borderRadius: 12, padding: '14px 0', fontWeight: 700, fontSize: 20, margin: '18px 0 0 0', boxShadow: '0 8px 24px rgba(34,197,94,0.13)', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10, cursor: 'pointer', transition: 'background 0.2s'}}>
            Se connecter <span style={{fontSize: 22, marginLeft: 8}}>&rarr;</span>
          </button>
